...
var config = {
  emojiVersion: '3.0', // you can [optionally] modify this to load alternate emoji versions. see readme for backwards compatibility and version options
  emojiSize: '32',
  spriteSize: '32',
...
config.imagePathPNG = 'https://cdn.jsdelivr.net/emojione/assets/' + config.emojiVersion + '/png/' + config.emojiSize + '/';
...
function shortnameToUnicode(str) {
    ...
    if (config.ascii) {
        var asciiRX = config.riskyMatchAscii ? regAsciiRisky : regAscii;
...
function shortnameToImage(str) {
    ...
    str = str.replace(regShortNames, function(shortname) {
        if( (typeof shortname === 'undefined') || (shortname === '') || (shortnames.indexOf(shortname) === -1) ) {
            ...
            title = config.imageTitleTag ? 'title="' + shortname + '"' : '';
            size = (config.spriteSize == '32' || config.spriteSize == '64') ? config.spriteSize : '32';

            // depending on the settings, we'll either add the native unicode as the alt tag, otherwise the shortname
            alt = (config.unicodeAlt) ? convert(unicode.toUpperCase()) : shortname;

            if(config.sprites) {
                replaceWith = '<span class="emojione emojione-' + size + '-' + category + ' _' + fname + '" ' + title + '>' + alt + '</span>';
            }
            else {
                replaceWith = '<img class="emojione" alt="' + alt + '" ' + title + ' src="' + config.imagePathPNG + '' + fname + '.png"/>';
            }
    ...
    if (config.ascii) {
        ...
        str = str.replace(regAscii, function(entire, m1, m2, m3) {
            ...
            title = config.imageTitleTag ? 'title="' + escapeHTML(m3) + '"' : '';
            size = (config.spriteSize == '32' || config.spriteSize == '64') ? config.spriteSize : '32';

            // depending on the settings, we'll either add the native unicode as the alt tag, otherwise the shortname
            alt = (config.unicodeAlt) ? convert(unicode.toUpperCase()) : escapeHTML(m3);

            if(config.sprites) {
                replaceWith = m2+'<span class="emojione emojione-' + size + '-' + category + ' _' + unicode +'"  ' + title + '>' + alt + '</span>';
            }
            else {
                replaceWith = m2+'<img class="emojione" alt="'+alt+'" ' + title + ' src="' + config.imagePathPNG + unicode + '.png"/>';
            }
...
function unicodeToImage(str) {
    ...
    str = str.replace(regUnicode, function(unicodeChar) {
        ...
        else if ( config.greedyMatch && unicodeChar in jsEscapeMapGreedy )
        {
        ...
        // depending on the settings, we'll either add the native unicode as the alt tag, otherwise the shortname
        alt = (config.unicodeAlt) ? convert(unicode.toUpperCase()) : short;
        title = config.imageTitleTag ? 'title="' + short + '"' : '';
...
export { asciiList, asciiRegexp, convert, emojioneList, escapeHTML, escapeRegExp, jsEscapeMap, jsEscapeMapGreedy, mapEmojioneList, mapUnicodeCharactersToShort, mapUnicodeToShort, memMapShortToUnicode, memMapShortToUnicodeCharacters, memorizeReplacement, objectFlip, regAscii, regShortNames, regUnicode, replaceAll, shortnameConversionMap, shortnameToAscii, shortnameToImage, shortnameToUnicode, shortnames, toImage, toShort, unescapeHTML, unicodeCharRegex, unicodeReplacementRegEx, unicodeToImage, unifyUnicode };export default config;
